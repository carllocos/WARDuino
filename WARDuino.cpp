            ...
            case 9: {
                ...
                for (uint32_t c = 0; c < element_count; c++) {
                    ...
                    for (uint32_t n = 0; n < num_elem; n++) {
                        debug(
                            "  write table entries %p, offset: 0x%x, n: "
                            "0x%x, "
                            "addr: %p\n",
...
// Proxy client
void WARDuino::addProxy(uint32_t fidx) {
    this->proxyClient.proxies.insert(fidx);
}

bool WARDuino::isProxy(uint32_t fidx) {
    return this->proxyClient.proxies.find(fidx) !=
           this->proxyClient.proxies.end();
}

void WARDuino::addProxyHost(char *host, int porno) {
    this->proxyClient.host = host;
    this->proxyClient.port = porno;
}

void WARDuino::clearProxyState() {
    this->proxyClient.proxies.clear();
    if (this->proxyClient.host != nullptr) free(this->proxyClient.host);
}
...
bool WARDuino::hasProxyCall() { return this->proxyServer.fun != nullptr; }

StackValue *WARDuino::getProxyArgs() { return this->proxyServer.args; }

Type *WARDuino::getProxyType() {
    if (this->proxyServer.fun == nullptr) return nullptr;
    return this->proxyServer.fun->type;
}
...
copyVals *copyStackValues(StackValue *vals, uint32_t amount);
...
void WARDuino::returnProxyCall(StackValue *v, bool success) {
    ...
    if (this->proxyServer.args != nullptr) {
        printf("freeing args\n");
        free(this->proxyServer.args);
    }

    copyVals *cp = nullptr;
    if (v != nullptr) {
        cp = copyStackValues(v, 1);
        uint8_t suc = success ? 1 : 0;
        unsigned char *raw =
            (unsigned char *)malloc(sizeof(unsigned char) * (cp->len + 1));
...
uint32_t WARDuino::getProxy() {
    if (this->proxyServer.fun != nullptr) return this->proxyServer.fun->fidx;
    return -1;
}

#ifndef SOCKET  // TODO remove
#define SOCKET 1
#endif
...
ProxyResult *WARDuino::proxyCall(Block *fun, StackValue *args) {
    printf("calling proxy\n");
    ProxyClient *pxc = &this->proxyClient;
    ProxyResult *pr = (ProxyResult *)malloc(sizeof(ProxyResult));
    if (pxc->host == nullptr) {
        sprintf(exception, "Proxy host not set");
        pr->succes = false;
        return pr;
    }

    copyVals *cp = copyStackValues(args, fun->type->param_count);
    size_t len = sizeof(uint32_t) + cp->len;
    unsigned char *raw = (unsigned char *)malloc(len);
    memcpy(raw, &fun->fidx, 4);
    memcpy(raw + 4, cp->raw, cp->len);
    char *tmphexa = tohex(raw, len);
    size_t total = len * 2 + 4;
    char *hexa = (char *)malloc(sizeof(char) * total);
    hexa[0] = '2';
    hexa[1] = '6';
    hexa[total - 1] = '\n';
    hexa[total] = '\0';
    memcpy(hexa + 2, tmphexa, len * 2);
...
#else
StackValue *WARDuino::proxyCall(uint32_t fidx, Module *m) { return nullptr; }
#endif
...
copyVals *copyStackValues(StackValue *vals, uint32_t amount) {
    ...
    copyVals *cpy = (copyVals *)malloc(sizeof(copyVals));
    cpy->len = vals_len;
    cpy->raw = raw;
    return cpy;
}
...
char *tohex(unsigned char *raw, size_t len) {
    size_t total = sizeof(char) * len * 2;
    char *hexa = (char *)malloc(total);
    for (size_t i = 0; i < len; i++) {
        unsigned c = raw[i] >> 4;
