...
#include <queue>
#include <map>
#include <inttypes.h> //TODO remove:needed for PRIu32
...
unsigned short int sizeserialization_rfc(const Type *);
unsigned short int sizeserialization_rfcallee(RFC*);
void arguments_copy(unsigned char*, StackValue *, uint32_t);
void chars_as_hexa(unsigned char *, unsigned char*, uint32_t);
...
RFC* RFC::registerRFCallee(uint32_t t_fid, Type *t_type, StackValue *t_args){
    RFC *rfc = new RFC(t_fid, t_type, t_args);
    callees.push(rfc);
    return rfc;
}

bool RFC::hasRFCallee(){
    return !callees.empty();
}

RFC* RFC::currentCallee(){
    return callees.front();
}

void RFC::removeRFCallee(){
    callees.pop();
}
...
RFC::RFC(uint32_t t_fid, Type *t_type, StackValue *t_args): fid(t_fid), args(t_args), type(t_type){
...
void RFC::returnResult(){
    struct SerializeData *rfc_result = this->serializeRFCallee();
    wa_proxy(rfc_result->raw, rfc_result->size);
}

/*
 *
 * output:  1 byte  | 4 bytes | sizeof(arg1.value_type) bytes | sizeof(arg2.value_type) bytes | ...
 *        interrupt | funID   | arg1.value 
 *
...
struct RFC::SerializeData * RFC::serializeRFC(){
    ...
    unsigned char* hexa = new unsigned char[hexa_size + 2]; //+2 for '\n' and '0' termination
...
struct RFC::SerializeData* RFC::serializeRFCallee(){
    const unsigned short serializationSize = sizeserialization_rfcallee(this);
    ...
    if(this->succes && this->type->result_count > 0){
        dbg_info("serializeRFCallee: success value size=%u \n", sizeof_valuetype(this->result->value_type));
        memcpy(raw + 1, &this->result->value, sizeof_valuetype(this->result->value_type));
    }
    else if(!this->succes)
    {
        dbg_info("serializeRFCallee: serializing exception\n");
        memcpy(raw + 1, &this->excpMsgSize, sizeof(uint16_t));
        memcpy(raw + 1 + sizeof(uint16_t), this->exceptionMsg, this->excpMsgSize);
    }
 ...
 */
unsigned short int sizeserialization_rfc(const Type *type) {
...
unsigned short int sizeserialization_rfcallee(RFC* callee) {
    if(!callee->succes)
        return 1 + sizeof(uint16_t) + callee->excpMsgSize;
...
void arguments_copy(unsigned char* dest, StackValue * args, uint32_t param_count){
    uint32_t offset = 0;
...
void RFC::deserializeRFCResult(){
    ...
    if(!this->succes) {
        ...
        memcpy(this->exceptionMsg, call_result + 1 + sizeof(uint16_t), msg_size);
        delete[] call_result;
        return;
    }
...
void chars_as_hexa(unsigned char *dest, unsigned char* source, uint32_t len_source) {
