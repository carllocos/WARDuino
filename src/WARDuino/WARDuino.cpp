      ...
      switch (opcode) {  // NOLINT(hicpp-multiway-paths-covered)
        ...
        case 0x0b:  // end
          ...
          debug(
              ...
              block->start_ptr, block->end_ptr, block->br_ptr, block->else_ptr);
          break;
      }
      skip_immediates(&pos);
    }
  ...
  while (pos < bytes_end) {
    ...
    switch (id) {
      ...
      case 1:
        ...
        m->types =
            (Type *)acalloc(m->type_count, sizeof(Type), "Module->types");

        for (uint32_t c = 0; c < m->type_count; c++) {
          Type *type = &m->types[c];
          type->form = read_LEB(&pos, 7);
          ASSERT(type->form == FUNC,
                 "%" PRIu32 " -th type def was not a function type", c);

          // read vector params
          type->param_count = read_LEB_32(&pos);
          type->params = (uint32_t *)acalloc(type->param_count,
                                             sizeof(uint32_t), "type->params");
          for (uint32_t p = 0; p < type->param_count; p++) {
      ...
      case 2: {
        ...
        for (uint32_t gidx = 0; gidx < import_count; gidx++) {
          ...
          do {
            ...
            if (resolvesym(import_module, import_field, external_kind, &val,
                           &err, !m->options.disable_strict_load)) {
              break;
            }
          ...
          switch (external_kind) {
            case 0x00:  // Function
            {
              ...
              m->function_count += 1;
              m->functions =
                  (Block *)arecalloc(m->functions, fidx, m->import_count,
            ...
            case 0x03:  // Global
            {
              ...
              m->globals = (StackValue *)arecalloc(
                  m->globals, m->global_count - 1, m->global_count,
      ...
      case 4: {
        ...
        m->table.entries = (uint32_t *)acalloc(m->table.size, sizeof(uint32_t),
      ...
      case 6: {
        ...
        for (uint32_t g = 0; g < global_count; g++) {
          ...
          m->global_count += 1;
          m->globals = (StackValue *)arecalloc(
              m->globals, gidx, m->global_count, sizeof(StackValue), "globals");
        ...
        for (uint32_t c = 0; c < element_count; c++) {
          ...
          if (!m->options.disable_memory_bounds) {
            ASSERT(offset + num_elem <= m->table.size,
                   "table overflow %" PRIu32 "+%" PRIu32 " > %" PRIu32 "\n",
        ...
        for (uint32_t s = 0; s < seg_count; s++) {
          ...
          if (!m->options.disable_memory_bounds) {
            ASSERT(offset + size <= m->memory.pages * PAGE_SIZE,
                   "memory overflow %" PRIu32 "+%" PRIu32 " > %" PRIu32 "\n",
                   offset, size, (uint32_t)(m->memory.pages * PAGE_SIZE));
          }
          dbg_info(
      ...
      case 12:
        ...
        dbg_warn("Parsing Data(12) section ", section_len);
        pos += section_len;
        break;
      default:
...
#ifndef ARDUINO
  this->debugger->disconnect_proxy();  // TODO should this be in unload module?
#endif
...
void WARDuino::free_module_state(Module *m) {
  ...
  if (m->callstack != nullptr) {
    for (int j = 0; j <= m->csp; j++) {
      ...
      if (f->block != nullptr &&
          (f->block->block_type == 0xfe || f->block->block_type == 0xff)) {
        free(f->block);
      }
    }
    m->callstack = nullptr;
